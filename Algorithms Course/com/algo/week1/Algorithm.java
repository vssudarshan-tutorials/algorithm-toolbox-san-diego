...
public class Algorithm {
    ...
    *  */
    ...
    public static long findSecondRecursive(long n, long[] A){
        ...
        long[] secondCompared = findMaxTournament(2, (int)firstCompared[0]-1, firstCompared); //log2(n)-1 comparisons.
        //Total comparisons: n+log2(n)-2;

        System.out.println("comparison: " + firstCompared[1] + "(" + 1 + ")" + " x " + secondCompared[1] + "(" + 1 + ")" + " = " + secondCompared[1] * firstCompared[1]);
        System.out.println("Array Length: " + n);
        return secondCompared[1] * firstCompared[1];
    }

    private static long[] findMaxTournament(int low, int high, long[] A){
        if(low == high){
            long[] compared = new long[2];
            compared[0] = 2;
            compared[1] = (int)A[low];
            return compared;
        }
        long[] compared1 = findMaxTournament(low, (low+high)/2, A);
        long[] compared2 = findMaxTournament((low+high)/2+1, high, A);
        if(compared1[1] > compared2[1]){
            int k = (int)compared1[0] + 1;
            long[] newcompared1 = new long[k];
            System.arraycopy(compared1, 0, newcompared1, 0,(int) compared1[0]);
            newcompared1[0] = k;
            newcompared1[k-1] = compared2[1];
            return newcompared1;
        }
        int k =(int) compared2[0] + 1;
        long[] newcompared2 = new long[k];
        System.arraycopy(compared2, 0, newcompared2, 0, (int)compared2[0]);
        newcompared2[0] = k;
